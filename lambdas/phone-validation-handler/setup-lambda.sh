...
# This script creates the IAM role and Lambda function for phone-validation-handler
...
# Configuration
FUNCTION_NAME="phone-validation-handler"
ROLE_NAME="mobile-phone-validation-lambda-role"
REGION="us-west-2"
ACCOUNT_ID=$(aws_cmd sts get-caller-identity --query 'Account' --output text)

echo "=========================================="
echo "Phone Validation Lambda Setup"
echo "=========================================="
...
if aws_cmd iam get-role --role-name "$ROLE_NAME" > /dev/null 2>&1; then
    ...
    aws_cmd iam create-role \
        ...
        --description "Execution role for phone-validation-handler Lambda function"
    ...
    # Create inline policy for phone validation permissions
    echo "  Creating inline policy for phone validation permissions..."
    POLICY_DOC='{
        "Version": "2012-10-17",
        "Statement": [
            {
                ...
                "Resource": [
                    "arn:aws:dynamodb:'"$REGION"':*:table/mobile-phone-verifications",
                    "arn:aws:dynamodb:'"$REGION"':*:table/mobile-phone-verifications/index/*",
                    "arn:aws:dynamodb:'"$REGION"':*:table/mobile-users",
                    "arn:aws:dynamodb:'"$REGION"':*:table/mobile-users/index/*",
                    "arn:aws:dynamodb:'"$REGION"':*:table/mobile-approved-emails"
                ]
            },
            {
                "Effect": "Allow",
                ...
                "Resource": [
                    "arn:aws:secretsmanager:'"$REGION"':*:secret:mobile-app/api-key*",
                    "arn:aws:secretsmanager:'"$REGION"':*:secret:mobile-app/telnyx-api-key*"
                ]
            }
        ]
    }'
    ...
    aws_cmd iam put-role-policy \
        --role-name "$ROLE_NAME" \
        --policy-name mobile-phone-validation-lambda-policy \
...
if [ ! -f "phone-validation-handler.zip" ]; then
...
if aws_cmd lambda get-function --function-name "$FUNCTION_NAME" --region "$REGION" > /dev/null 2>&1; then
    ...
    echo "  To update code, run: ./deploy-lambda-s3.sh"
else
    ...
    aws_cmd lambda create-function \
        ...
        --zip-file fileb://phone-validation-handler.zip \
        --timeout 30 \
        --memory-size 512 \
        --environment Variables="{
            VERIFICATIONS_TABLE=mobile-phone-verifications,
            APPROVED_EMAILS_TABLE=mobile-approved-emails,
            USERS_TABLE=mobile-users,
            API_KEY_SECRET_NAME=mobile-app/api-key,
            TELNYX_API_KEY_SECRET_NAME=mobile-app/telnyx-api-key
        }" \
        --region "$REGION" \
        --description "Handles phone number validation via SMS verification codes"
...
echo "3. Configure API Gateway routes (run ./setup-api-gateway.sh)"
echo ""

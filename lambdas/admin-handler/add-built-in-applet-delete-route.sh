...
# Add DELETE /v1/admin/applets/built-in/{appletId} route to API Gateway.
# Used by the admin "Apps" tab to hard-delete a built-in applet. Idempotent
# - safe to re-run; existing resources / methods / permissions are skipped.
# Run once after shipping the admin-handler Lambda update that adds
# handleDeleteBuiltInApplet.
#
# This script assumes add-built-in-applet-color-route.sh has already been run
# (the {appletId} resource must already exist).
...
DELETE_EXISTS=$(aws_cmd apigateway get-method \
    --rest-api-id "$API_ID" \
...
if [ "$DELETE_EXISTS" == "None" ]; then
    echo "📝 Creating DELETE method on /admin/applets/built-in/{appletId}..."
...
STATEMENT_ID="apigateway-${API_ID}-built-in-applet-delete"
SOURCE_ARN="arn:aws:execute-api:${REGION}:${ACCOUNT_ID}:${API_ID}/*/DELETE/admin/applets/built-in/*"
...
DEPLOYMENT_ID=$(aws_cmd apigateway create-deployment \
    ...
    --stage-name v1 \
    --region "$REGION" \
    --description "Add DELETE /admin/applets/built-in/{appletId}" \
    --query 'id' \
    --output text)
...
echo ""
echo "📋 Summary"
echo "   Route:        DELETE /v1/admin/applets/built-in/{appletId}"

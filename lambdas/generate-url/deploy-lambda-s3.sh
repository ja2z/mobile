...
# Configuration
FUNCTION_NAME="generateSigmaEmbedURL"
REGION="us-west-2"
S3_BUCKET="mobile-lambda-deployments"
S3_KEY="generate-url/${FUNCTION_NAME}-$(date +%Y%m%d-%H%M%S).zip"
ZIP_FILE="generateSigmaEmbedURL.zip"
...
echo "   aws s3 rm s3://${S3_BUCKET}/generate-url/ --recursive --exclude '*.zip' --include '*.zip' --dryrun"

...
# This script performs a clean build of the Lambda function and creates phone-validation-handler.zip
...
if [ -f "phone-validation-handler.zip" ]; then
  rm -f phone-validation-handler.zip
  echo "  ✓ Removed phone-validation-handler.zip"
fi
...
# TypeScript preserves directory structure, so index.js is in dist/phone-validation-handler/
# We need to copy it to root and fix the require paths for shared modules
if [ -f "dist/phone-validation-handler/index.js" ]; then
  cp dist/phone-validation-handler/index.js index.js
...
echo ""
echo "🗜️  Creating phone-validation-handler.zip..."
...
zip -rq phone-validation-handler.zip node_modules index.js shared
...
echo ""
echo "✅ Build complete! phone-validation-handler.zip is ready for upload."
echo "📦 File size: $(du -h phone-validation-handler.zip | cut -f1)"

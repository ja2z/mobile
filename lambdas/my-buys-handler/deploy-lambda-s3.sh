...
S3_KEY="my-buys-handler/${FUNCTION_NAME}-$(date +%Y%m%d-%H%M%S).zip"
ZIP_FILE="my-buys-handler.zip"
...
echo "   aws s3 rm s3://${S3_BUCKET}/my-buys-handler/ --recursive --exclude '*.zip' --include '*.zip' --dryrun"

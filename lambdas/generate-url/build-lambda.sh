...
# This script performs a clean build of the Lambda function and creates generateSigmaEmbedURL.zip
...
# TypeScript preserves directory structure, so index.js is in dist/generate-url/
# We need to copy it to root and fix the require paths for shared modules
if [ -f "dist/generate-url/index.js" ]; then
  cp dist/generate-url/index.js index.js

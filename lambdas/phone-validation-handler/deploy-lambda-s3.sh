...
S3_KEY="phone-validation-handler/${FUNCTION_NAME}-$(date +%Y%m%d-%H%M%S).zip"
ZIP_FILE="phone-validation-handler.zip"
...
echo "   aws s3 rm s3://${S3_BUCKET}/phone-validation-handler/ --recursive --exclude '*.zip' --include '*.zip' --dryrun"

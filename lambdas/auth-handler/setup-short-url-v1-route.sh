...
# Setup API Gateway route for short URL redirect endpoint at /v1/s/{shortId}
...
echo "🔍 Getting /v1 resource ID..."
V1_ID=$(aws_cmd apigateway get-resources \
    --rest-api-id $API_ID \
    --region $REGION \
...
if [ -z "$V1_ID" ]; then
    echo "❌ Error: Could not find /v1 resource"
    exit 1
fi

echo "✓ Found /v1 resource ID: $V1_ID"
echo ""

# Check if /v1/s resource exists
echo "🔍 Checking if /v1/s resource exists..."
S_RESOURCE_ID=$(aws_cmd apigateway get-resources \
    --rest-api-id $API_ID \
    --region $REGION \
    --query "items[?path=='/v1/s'].id" \
    --output text)

if [ -z "$S_RESOURCE_ID" ]; then
    echo "📝 Creating /v1/s resource..."
    S_RESOURCE_ID=$(aws_cmd apigateway create-resource \
        ...
        --query 'id' \
        --output text)
    echo "✓ Created /v1/s resource: $S_RESOURCE_ID"
else
    echo "✓ /v1/s resource already exists: $S_RESOURCE_ID"
fi
echo ""

# Check if /v1/s/{shortId} resource exists
echo "🔍 Checking if /v1/s/{shortId} resource exists..."
SHORT_ID_RESOURCE_ID=$(aws_cmd apigateway get-resources \
    --rest-api-id $API_ID \
    --region $REGION \
    --query "items[?path=='/v1/s/{shortId}'].id" \
    --output text)

if [ -z "$SHORT_ID_RESOURCE_ID" ]; then
    echo "📝 Creating /v1/s/{shortId} resource..."
    SHORT_ID_RESOURCE_ID=$(aws_cmd apigateway create-resource \
        ...
        --query 'id' \
        --output text)
    echo "✓ Created /v1/s/{shortId} resource: $SHORT_ID_RESOURCE_ID"
else
    echo "✓ /v1/s/{shortId} resource already exists: $SHORT_ID_RESOURCE_ID"
fi
echo ""
...
aws_cmd lambda add-permission \
    ...
    --statement-id apigateway-invoke-short-url-v1 \
...
DEPLOYMENT_ID=$(aws_cmd apigateway create-deployment \
    ...
    --stage-name v1 \
    --region $REGION \
    --description "Add short URL redirect endpoint /v1/s/{shortId}" \
    --query 'id' \
    --output text)
...
echo "   Accessible via: https://mobile.bigbuys.io/v1/s/{shortId}"
